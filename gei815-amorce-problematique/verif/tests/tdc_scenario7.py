...
@cocotb.test()
async def tdc_scenario7(dut):

    init.initDebug("TDC scenario7 - CHECK MAX 5us")

    #FROM design/digital/UART/packet_merger.sv
    TDC = MMC.MMC_TDC(dut)
    TDC.start()
    TDC.message_queue = {'time_tested' : [6,'us'] ,'isTestSupposedToFail' : False} # 2.5 us
...
async def wait_reply(dut, uart_sink):
    ...
    if(x == 99):
        ...
        return cocotb.binary.BinaryValue(value=message_bytes, n_bits=48, bigEndian=False)